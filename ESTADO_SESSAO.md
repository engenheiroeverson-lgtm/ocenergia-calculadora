...
- **Pipeline de sync (NÃO MEXER — está correto):**
  ...
  - `[skip ci]` REMOVIDO da mensagem de commit do robô (2026-06-24): sync semanal agora dispara deploy Vercel automaticamente e publica o JSON atualizado sem precisar de um commit de código subsequente.
---
...
1. ✅ **Validar a Híbrida na tela** — concluído em produção (24/06/2026).
2. ✅ Workflow ANEEL: `grafias-aneel.json` já em `public/`; `[skip ci]` REMOVIDO do commit do robô.
3. (Opcional) Configurar env `WHATSAPP_ENGENHARIA_WEBHOOK_URL`.
4. Plugar parser real de PDF de fatura (hoje stub; badge honesto). **Próximo prioritário substantivo — exige design (escolha de lib PDF, mapa de formatos por concessionária, suíte de testes) antes de codar.**
