...
> Última atualização: 2026-06-24 (Híbrida FP validada em produção · sync ANEEL auto-publica)
...
- **Pipeline de sync (NÃO MEXER — está correto):**
  ...
  - `.github/workflows/sincroniza-grafias-aneel.yml`: cron seg 06:00 UTC + dispatch manual; `git add public/grafias-aneel.json` + commit/push. **`[skip ci]` removido (2026-06-24)** — cada sync com diff agora auto-publica via redeploy Vercel. Sem risco de loop (GITHUB_TOKEN padrão não reentra no próprio workflow).
---
...
1. ✅ **Validar a Híbrida na tela** — validada em produção (2026-06-24). Decisão pendente (futura): se ela se torna a calculadora padrão ou se as duas convivem.
2. ✅ **Sync ANEEL → `public/` + sem `[skip ci]`** — script já gravava em `public/` (estável desde 2026-06-22); `[skip ci]` removido em 2026-06-24, agora auto-publica.
3. (Opcional) Configurar env `WHATSAPP_ENGENHARIA_WEBHOOK_URL`.
4. **Plugar parser real de PDF de fatura** (hoje stub; badge honesto). → Próximo prioritário do backlog técnico.
...
- `cursor/remover-skip-ci-sync-grafias-aneel-116d` chore(ci): remove [skip ci] do commit de sync das grafias ANEEL — PR aberto (auto-publica a cada sync semanal)
- `20fd329` Update TelaPrincipal.tsx — **READY** (Híbrida plugada como 4ª sub-aba; validada em produção 2026-06-24)
